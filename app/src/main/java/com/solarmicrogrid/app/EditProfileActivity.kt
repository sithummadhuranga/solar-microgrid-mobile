    ...
    private fun loadProfile() {
        Thread {
            try {
                ...
                runOnUiThread { showMessage(e.message ?: getString(R.string.error_could_not_load)) }
            }
        }.start()
    }
    ...
    private fun saveProfile() {
        ...
        Thread {
            try {
                ...
                runOnUiThread { showMessage(e.message ?: getString(R.string.error_could_not_save)) }
            }
        }.start()
    }

    // checks both password fields are filled, then sends the change request off the main thread
